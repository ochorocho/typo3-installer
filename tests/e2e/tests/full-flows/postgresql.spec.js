...
/**
 * PostgreSQL Full Installation Flow
 *
 * Tests complete TYPO3 installation with PostgreSQL database.
 * Includes WCAG 2.2 accessibility checks at each step (logged, not blocking).
 *
 * Prerequisites:
 * - DDEV environment running with PostgreSQL container
 * - Empty database (global-setup.js handles reset)
 */
test.describe.serial('PostgreSQL Full Installation Flow', () => {
  ...
  test('complete installation with PostgreSQL and WCAG checks', async ({ page }) => {
    ...
    await fillPostgreSQLForm(page);

    // Wait for auto-validation to complete (shows success alert)
    // PostgreSQL validation might take longer
